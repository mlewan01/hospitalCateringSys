<?php
  ...
  if(isset($_POST['l_savepass'])){
    ...
    if(sanitise($newpass)){
      ...
      if (!$re) {
        $content .= 'Link does not exist. Pleas try password recovery again.'.$b;
      ...
      $content .= "Password contains illegal characters, pleas try again. Only alpha-numeric and this characters allowed: '@', ' ', '.', ',', '?', '!',':', '-', '&', '_'";
    }
...
min length 8" required aria-required="true" aria-labbelledby="l2_password"/>';
    ...
    if(isset($_POST['l2_username'])){
      ...
      $content .= "Username contains illegal characters, pleas try again. Only alpha-numeric and this characters allowed: '@', ' ', '.', ',', '?', '!',':', '-', '&', '_'";
    }
    }else{
